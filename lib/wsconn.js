...
function register(socket, msg) {
  ...
  if (editId) {
    ...
    broadcast(editId, userId, `operator:{"userId":${userId},"operator":"${name}"}`);
...
function deregister(editId, userId) {
  ...
  if (editId && editor[editId]) {
    ...
    // broadcast user to others
    broadcast(editId, userId, `operator:{"userId":${userId},"operator":""}`);
...
function broadcast(editId, userId, msg) {
  ...
  if (msg.startsWith('edit:')) {
    ...
    if (save || type === 'title' || type === 'mode') {
      ...
      // store user ID
      editor[editId].user[data.userId].name = data.operator;
